/*
 * This software manages serial communication in order to drive connected led strips.
 * Implementation and tests were made on HC-05 Bluetooth module and WS2812b led strip. 
 * Any other variations may be possible but weren't tested.
 * For LED management and step iteration is used NeoAnimations class linked below.
 * Commmunication is made in standard Arduino way with usage SoftwareSerial lib.
 * There are preset LED illuminations that can be adjusted but definition of them must 
 * be declared on the host and client side which means if someone wants add new illumination, 
 * it's name must be added to the list on transmitter device.
 * Code uses own communications messages.
 * 
